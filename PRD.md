...
24. ~~As a user facing a hard failure (not a transient one), I want a blocking dialog only in that case, so that minor hiccups don't interrupt me but serious problems are clearly flagged.~~ **Superseded for the Weather PoC Feature** (confirmed with the user during that Feature's Spec, 2026-07-08): every fetch failure in this PoC's scope — Open-Meteo/ip-api.com unreachable, no network — is transient, so no hard-failure case exists to trigger this dialog. Requirement kept here, not implemented, in case a future Feature introduces a genuine hard-failure case (e.g. corrupted local persistence). (Corrected 2026-07-09: the IP-geolocation fallback is `ip-api.com`, not BigDataCloud — live-verified during that Feature's Spec seam enumeration; BigDataCloud's geolocation endpoint requires an API key and is not actually keyless.)
